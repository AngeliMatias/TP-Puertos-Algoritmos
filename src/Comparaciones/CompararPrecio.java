...
public class CompararPrecio implements Comparar{
    public boolean comparar(NodoGrafo nodoEvaluar, NodoGrafo nodoAdyacente, Viaje arista){
        return nodoEvaluar.getDistancia()+arista.getCosto() < nodoAdyacente.getDistancia();
    }

    @Override
    public void modificar(NodoGrafo nodoEvaluar, NodoGrafo nodoAdyacente, Viaje arista) {
        nodoAdyacente.setDistancia(nodoEvaluar.getDistancia() + arista.getCosto());
    }
    public void actualizarCamino(NodoGrafo nodoEvaluar, NodoGrafo nodoAdyacente, Viaje arista, CaminoMinimo camino){
        ...
        if (nodoCamino==null /*|| nodoCamino.getPesoCamino() == nodoAdyacente.getDistancia() */){
            nodoCamino = new NodoCamino(nodoAdyacente.getDato(), nodoAdyacente.getDistancia(),arista.getCosto(),nodoEvaluar.getDato().getCodigo());
            camino.agregarNodo(nodoCamino);
        }else{
            nodoCamino.setPadre(nodoEvaluar.getDato().getCodigo());
            nodoCamino.setDistanciaPadre(arista.getCosto());
